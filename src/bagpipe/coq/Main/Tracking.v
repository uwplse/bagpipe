...
Section Tracing.
  ...
  Instance trackingAttributes : PathAttributesClass. 
    refine {|
      ...
      leDecPathAttributes a a' := leDecPathAttributes (current a) (current a')
    |}.
  Proof.
    intro a.
  ...
  Instance trackingConfiguration r : ConfigurationClass r.
    refine {|
      ...
      bestIncoming f := bestIncoming r (fun i => match f i with 
                                                 | notAvailable => notAvailable  
                                                 | available a => available (current a) 
                                                 end)
    |}.
  Proof.
    - intros.
      ...
      branch; [|congruence].
      match goal with |- match f (bestIncoming r ?F) with _ => _ end = true => specialize (bestIncomingIsBest r F i); intro b end.
      match goal with h:f i = _ |- _ => rewrite h in b; clear h end.
      revert b.
      branch; cbn; congruence.
  Defined.
  ...
  Definition trackingOk r (s:incoming r) (p:Prefix) (a:@RoutingInformation trackingAttributes) : Prop.
    refine(match a with
    | notAvailable => True
  ...
  Hint Unfold routerHandler messageHandling decisionProcess updateSendProcess 
              import' export' originate' bindRoutingInformation connection emptyRIB.

  Lemma trackingOk' : forall r s d (c:connection r d) p a,
    trackingOk r s p a -> 
    trackingOk d (received [r & c]) p (export' r s [d & c] p (import' r s p a)).
  Proof.
    intros.
  ...
  Lemma reachableImpliesTrackingOk :
    forall ns (R:reachable ns), (
      ...
      forall s d (c:connection s d),
        ...
        forall m (mInMs:In m ms), trackingOk d (received [s & c]) (nlri m) (attributes m)
    ).
  Proof.
    intros.
      ...
      + (* gen *)
        ...
        * destruct m as [p a].
          ...
          break_if. {
            ...
            destruct (p' =? p'); crush.
            match goal with
            | |- context[bestIncoming ?R ?A] => generalize (bestIncoming R A); intro i
            end.
            unfold lookup.

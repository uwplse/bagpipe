...
Section SingleAS.
  ...
  Class SingleASTopologyClass := {
    ...
    SingleASUninterpretedState : Type;
    singleASInitialUninterpretedState : SingleASUninterpretedState
  }.
  ...
  refine {|
    ...
    mode := singleASMode;
    UninterpretedState := SingleASUninterpretedState;
    initialUninterpretedState := singleASInitialUninterpretedState
  |}.
  - intros.
    constructor.
  ...
  Class SingleASConfigurationClass := {
    ...
    bestIncoming' : forall r, (incoming r -> RoutingInformation) -> incoming r;
    bestIncomingBest' : forall r f i, leDecRoutingInformation (f i) (f (bestIncoming' r f)) = true
  }.
  ...
  Instance singleASConfiguration : forall r:Router, ConfigurationClass r.
    ...
    - refine {|
        ...
        originate p a := false;
        bestIncoming f := bestIncoming' [internal & r] f
      |}.
      apply bestIncomingBest'.
    - refine {|
        import i p a := match i with injected => available a | _ => notAvailable end;
        export i o p a := available a;
        originate p a := true;
        bestIncoming f := bestIncoming' [external & r] f
      |}.
      apply bestIncomingBest'.
  Defined.
End SingleAS.

...
Section BGP.
  ...
  *)
  ...
  Class PrefixClass := {
    ...
    enumerablePrefix :> enumerable Prefix
  }.
  Context `{PrefixClass}.
  ...
  Class PathAttributesClass := {
    ...
    leDecPathAttributes : PathAttributes -> PathAttributes -> bool;
    leDecPathAttributesEq : forall a, leDecPathAttributes a a = true
  }.
  ...
  Global Instance eqDecRoutingInformation : eqDec RoutingInformation.
    constructor. decide equality. apply eqDecide. 
  Defined.

  Global Instance enumerableRoutingInformation `{enumerable PathAttributes} : enumerable RoutingInformation.
  ...
  Definition leDecRoutingInformation (a a':RoutingInformation) : bool :=
    match a,a' with
    | notAvailable,_ => true
    | available _, notAvailable => false
    | available a, available a' => leDecPathAttributes a a'
    end.

  Inductive Mode := ibgp | ebgp.
  Global Instance eqDecMode : eqDec Mode. 
  ...
  Class TopologyClass := {
    ...
    UninterpretedState : Type;
    initialUninterpretedState : UninterpretedState
  }.
  ...
  *)
  ...
  Global Instance eqDecUpdateMessage : eqDec UpdateMessage.
    idtac.
    constructor. 
  ...
  Section RouterInternal.
    ...
    Global Instance eqDecIncoming : eqDec incoming.
      constructor. 
      intros i i'.
      destruct i, i'.
      - left. reflexivity. 
      ...
      - decide equality. apply eqDecide.
    Defined.
    Global Instance enumerableIncoming : enumerable incoming.
      refine {|
    ...
    Class ConfigurationClass := {
      ...
      (* 9.4.  Originating BGP routes
         ...
         Looking at selfnet's config, it appears that there simply is no import filter 
      ...
      *)
      ...
      bestIncoming : (incoming -> RoutingInformation) -> incoming;
      bestIncomingIsBest : forall f i, leDecRoutingInformation (f i) (f (bestIncoming f)) = true
    }.
    ...
    Definition decisionProcess (p:Prefix) (ribs:RIBs) : RIBs.
      ...
      *)
      ...
      (* 9.1.2.  Phase 2: Route Selection
         ...
         > held in the Loc-RIB.
      *)
      let n' := bestIncoming (fun s => import' s p (lookup (adjRIBsIn ribs) (s,p))) in
  ...
  Record NetworkState : Type := networkState {
    ...
    uninterpretedState : UninterpretedState
  }.
  ...
  Fixpoint changeHandler (ps:list Prefix) (r:Router) (rs:forall r, RIBs r) {struct ps} : 
                         dict Connection (list UpdateMessage) * (forall r, RIBs r).
    refine (match ps with 
    | [] => (const [], rs)
    | p :: ps' => _
    end).
    refine (let Ers' := routerHandler r (injected r) 
                                      (updateMessage p (adjRIBsIn r (rs r) (injected r,p))) 
                                      rs in _).
    refine (let E' := fst Ers' in _).
    refine (let rs' := snd Ers' in _).
    refine (let Ers'' := changeHandler ps' r rs' in _).
    refine (let E'' := fst Ers'' in _).
    refine (let rs'' := snd Ers'' in _).
    refine ((fun c => E' c ++ E'' c), rs'').
  Defined.
  ...
  | forward m c rs ls us :
      ...
      Transition
        ...
        {| routerState := rs'; linkState := build (fun c => lookup ls c ++ lookup E c); uninterpretedState := us |}
  | change r rs ls us us' :
      let Ers' := changeHandler enumerate r rs in 
      let E := fst Ers' in
      let rs' := snd Ers' in
      Transition
        {| routerState := rs;  linkState := ls; uninterpretedState := us |}
  ...
  Definition emptyNetworkState := {|
    ...
    uninterpretedState := initialUninterpretedState
  |}.
End BGP.

...
Require Import Coq.Init.Datatypes. (* to prevent shadowing *)
...
Global Instance enumerableSigT {A B} `{eqDec A} `{enumerable A} 
...
Global Instance enumerableProd {A B} `{eqDec A} `{enumerable A} `{eqDec B} `{enumerable B} : enumerable (A * B).
  refine {|
    enumerate := bindList 
...
Global Instance enumerableEmptySet : enumerable Empty_set.
  refine {|enumerate := []|}.
Proof.
  crush.
Defined.

Global Instance enumerableUnit : enumerable unit.
  refine {|enumerate := [tt]|}.
Proof.
  crush.
Defined.
...
Global Instance enumerableSum {A B} `{enumerable A} `{enumerable B} : enumerable (A + B).
...
Proof.
  ...
  - intros.
    ...
    break_match; [crush|].
    generalize_proofs.
    assert (a0 :: l <> []) as p by congruence.
    specialize (IHl a p).
    subst_max.
    break_match.
    + specialize (leb_complete _ _ Heqb); intro; clear Heqb.
      destruct H' as [|H'].
      * subst_max. omega.
      * specialize (IHl H'). simpl in *. omega.
...
Global Instance enumerableFull {A} {h:forall (S: Basic), @Full S A} : enumerable A.
  idtac.
  specialize (h listSpace).
  destruct h.
  cbn in *.
  refine {|enumerate := full|}.
Proof.
  rewrite fullIsTrue in denoteFullOk.
  intro a.
  apply equal_f with (x := a) in denoteFullOk.
